...
2.  **Native Skill Integration:** If your host IDE/Engine supports an integrated skill/rules folder (e.g., `.cursor/rules/`, `.github/copilot-instructions/`), you MUST explicitly map, copy, or reference the applicable markdown files from AAIG's `skills/` directory into that native integration folder so the environment discovers them on-demand for all future tasks.
