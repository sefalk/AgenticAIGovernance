...
updated: 2026-07-29
status: DRAFT — text ready, GIF rendered
---
...
| File | Use |
|---|---|
| `assets/red-phase-block.gif` | 1280×600, 5 frames, 17.6 s, loops. ~91 KB. |
| `assets/red-phase-block-fallback.png` | Frame 4 as a still. Use if embedded video fails. |

Both are generated by [render-red-phase-gif.py](render-red-phase-gif.py)
(Pillow, Consolas). Edit the `FRAMES` list in that script and re-run to change
wording, colours or timing — do not hand-edit the images.

```text
python render-red-phase-gif.py
```

Frame timings are set per frame in `DURATIONS`: the dispatch and the hook get
the most time, the passing run the least. If the talk runs long, the whole GIF
can be cut to frames 3–5 without losing the argument.
...
- **Consistent dates.** Every artefact is stamped `2026-08-10` — the talk date.
  ...
  front row notices.
- **Fallback frame.** `assets/red-phase-block-fallback.png` is frame 4 exported
  as a still. If the GIF fails to play, that single frame carries the argument
  on its own.
- **Regenerating.** All GIF content lives in the `FRAMES` list of
  [render-red-phase-gif.py](render-red-phase-gif.py). Changing a line of text is
  a one-line edit plus a re-run; the images are build output, not source.
