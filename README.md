...
1.  **Drop the files in your repo:** Copy `AgenticAIGovernance.md`, `Level2-SoftwareDevelopment.md`, `AAIG_Assimilation_Protocol.md`, and the `/skills` folder into the root (or an `.agents/` folder) of your project repository.
...
Before starting ANY complex task, silently read `AAIG_Assimilation_Protocol.md` to understand your environmental capabilities and bootstrap your operations. Once assimilated, follow the Level-2 Domain Rules and utilize the `skills/` toolbox for specific technical tasks.
```
...
> *"Run the Assimilation Protocol at `AAIG_Assimilation_Protocol.md` to discover this environment, build the Level 4 Instantiation file, and then build [YOUR FEATURE]."*

### 3. CI/CD Pipeline Bots (e.g., PR Reviewers)
If you are using an LLM to automatically review Pull Requests via GitHub Actions:
1. Pass the contents of `Level2-SoftwareDevelopment.md` and `skills/code_quality/code_review.md` directly into the System Prompt of the LLM API call.
...
*   **Level 1: Core Principles (`AgenticAIGovernance.md`)** - Universal rules like *Separation of Concern* and *Fail-Safe*. They rarely change.
*   **Level 2: Domain Rules (`Level2-SoftwareDevelopment.md`)** - The L1 principles translated into 20 concrete `SHALL / SHALL NOT` rules specifically for building software.
...
*   **The Skills Toolbox (`/skills/`)** - A library of 47 "Just-in-Time" contextual skills. You don't load all 47 into the agent's context window. The agent loads `frontend_architecture.md` only when touching the UI, saving tokens while enforcing rigorous, specialized Quality Gates.
