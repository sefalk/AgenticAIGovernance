...
related: [model_evaluation, feature_engineering, experiment_tracking, exploratory_data_analysis]
---
# Model Selection & Architecture

## Purpose

Model selection is the process of choosing the right model architecture and algorithm family for a given task, data modality, and set of constraints. This skill covers the decision tree from problem type to model family, trade-offs between model classes, and practical guidance on when to use classical ML vs. deep learning. Invoke this skill when starting a new ML project, evaluating architecture alternatives, or defining Level-3 modeling workflows.

## Principles

- **Efficiency (AAIG L1):** Start simple. A logistic regression that solves the problem is better than a transformer that solves it 1% better at 100x the cost.
- **Verifiability (AAIG L1):** Model choice must be justified and documented. "We used XGBoost" is not a justification. "We used XGBoost because the data is tabular, moderate-sized, and we need feature importance" is.
- **Transparency (AAIG L1):** Model complexity must be proportional to the explainability requirements. High-stakes decisions may require interpretable models regardless of accuracy trade-offs.
- **Empirical validation:** No model is chosen by theory alone. Always benchmark candidates on your data. Conventional wisdom ("deep learning needs big data") has many exceptions.
- **Research before defaulting:** The ML field evolves rapidly. The decision tree in this skill is a starting point, not a ceiling. For any non-trivial project, the agent should survey current benchmarks (Papers With Code, arXiv, HuggingFace leaderboards) and community best practices for the specific task *before* committing to an architecture. New methods not yet covered here may outperform the listed defaults.
