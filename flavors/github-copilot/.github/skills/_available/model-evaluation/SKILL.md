...
description: Systematically measure model performance, fairness, and generalization — metric selection by task, validation strategies, confidence intervals, bias/fairness auditing, and explainability.
argument-hint: '[task: classification|regression|ranking|clustering|timeseries] [focus: metrics|validation|fairness|explainability]'
---
...
- When defining evaluation frameworks for ML models
- When comparing model candidates on held-out data
- When auditing models for bias and fairness
- When establishing statistical rigor (confidence intervals, significance tests)

## Principles

- **Verifiability:** Model performance claims must be backed by reproducible metrics on held-out data. No "the model works well" without numbers.
- **Transparency:** Evaluation methodology, including metric choices and dataset splits, must be documented and justified.
