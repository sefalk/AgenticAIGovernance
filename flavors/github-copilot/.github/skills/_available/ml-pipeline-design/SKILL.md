...
description: End-to-end ML workflow orchestration — pipeline architecture, model serving patterns, model registry, deployment strategies, drift monitoring, and retraining triggers.
argument-hint: '[focus: architecture|serving|deployment|monitoring|registry] [pattern: batch|realtime|edge]'
---
...
- When designing end-to-end training pipelines (data → model → deployment)
- When choosing model serving infrastructure (REST, batch, streaming, edge)
- When setting up model registries, versioning, and promotion workflows
- When implementing drift monitoring and automated retraining

## Principles

- **Verifiability:** Every pipeline step must have defined inputs, outputs, and quality gates. A pipeline that "just runs" without validation is a risk.
- **Transparency:** Pipeline lineage must be fully traceable: from raw data to deployed model, every transformation, parameter, and version is recorded.
- **Continuous Improvement:** Models degrade over time (data drift, concept drift). Pipelines must include monitoring and automated retraining triggers.
