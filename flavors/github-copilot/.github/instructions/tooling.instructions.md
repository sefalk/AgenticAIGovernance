...
Do not add comments. Per-task explanation goes in the `detail` field — it is
data, survives tooling round-trips, and shows in the task picker; cross-cutting
rules go in this file. A pre-commit guard
(`.github/hooks/scripts/check-strict-json.py`) rejects commits staging a
`tasks.json` that is not strict JSON. Deliberate override:
...
`skills/metrics/SKILL.md`. Renaming one breaks `run_task` calls at runtime with
no compile-time signal. Before renaming or removing a label, grep the payload
and update every reference in the same commit. Prefer **adding** a task over
renaming one.
...
Never `"command": "<tool>"` for a Python-provided tool. Routing through a runner
script has a second benefit: the rule set and scope stay single-sourced. A task
calling `ruff` directly bypasses `LINTING_STRICTNESS` and silently lints with
different rules than the quality gate.
...
surface into scratch. One script, varying arguments, one label per meaningful
scope: if you are appending `v2` to a label, you are parameterising by copy.
...
trail of what they did — execution is recorded in the workflow log and the hook
decisions, so leftover scratch labels should be pruned rather than read as
history. The documenter stop hook
(`.github/hooks/scripts/check-scratch-tasks.py`) reports the leftovers at
workflow end. It is advisory and never blocks: it lists entries that could not
be created today — inline interpreter payloads and prompt-resolved variables —
so a human can prune them.
