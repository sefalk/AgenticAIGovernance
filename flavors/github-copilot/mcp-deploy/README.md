...
A Model Context Protocol (MCP) server that exposes the AF framework deploy as
MCP tools, so an agent can inspect, preview **and apply** a rollout **without
cloning the AF repository next to the target project**.
...
> exactly like the scripts, so the AF repo need not sit next to the target.
...
| `AF_PAYLOAD_CACHE` | optional cache dir (default `~/.cache/af-deploy-mcp`) |
...
```bash
python -m build --wheel        # produces dist/af_deploy_mcp-*.whl
```
...
No AF clone is required — VS Code launches the packaged server locally (stdio)
...
```jsonc
{ "servers": { "af": { "type": "stdio", "command": "uvx", "args": ["af-deploy-mcp"] } } }
```
...
Then in chat: *"Use status for `${workspaceFolder}`"* and *"Run dry_run for
